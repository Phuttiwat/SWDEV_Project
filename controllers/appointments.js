...
exports.getAppointment = async (req,res,next)=>{
    try{
        ...
        if(!appointment){
            return res.status(404).json({success:false,message:`No appointment with the if of ${req.params.id}`});
        }
...
exports.addAppointment = async (req,res,next)=>{
    ...
    try{
        ...
        if(existedAppointments.length >=3 && req.user.role !== 'admin'){
            return res.status(400).json({succes:false,message:`The user with ID ${req.user.id} has already made 3 appointments`});
        }
...
exports.updateAppointment=async (req,res,next)=>{
    try{
        ...
        if(!appointment){
            return res.stataus.json({success:false,message:`No appointment with the id of ${req.params.id}`});
        }
